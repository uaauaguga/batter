# adapted from https://github.com/lonePatient/BERT-NER-Pytorch/blob/master/models/layers/crf.py with some modifications
import torch
...
class CRF(nn.Module):
    ...
    Attributes:
        start_transitions: start transition score tensor: (num_tags,)
        end_transitions: end transition score tensor: (num_tags,)
        transitions: transition score tensor: (num_tags, num_tags)
    """
    ...
    def _viterbi_decode(self, emissions: torch.FloatTensor,
        ...
        tags = torch.where(mask, best_tags_arr, oor_tag).transpose(0, 1)
        scores = (end_score/(seq_ends+1).view(batch_size,-1)).transpose(0, 1)
        return tags.unsqueeze(0), scores
    ...
    def _viterbi_decode_nbest(self, emissions: torch.FloatTensor,
        ...
        for i in range(1, seq_length):
            if i == 1:
                broadcast_score = score.unsqueeze(-1) # (batch_size, num_tags, 1)
            ...
            if next_score.shape[1] < nbest:
                # handle conditions that number of tags < nbest
                patch = torch.full((batch_size, nbest-next_score.shape[1],self.num_tags),-float("Inf")).to(next_score.device)
                next_score = torch.cat([next_score, patch],axis=1)
            next_score, indices = next_score.topk(nbest, dim=1)
        ...
        end_score, end_tag = end_score.view(batch_size, -1).topk(nbest, dim=1)
        # shape: (batch_size,)
        seq_ends = mask.long().sum(dim=0) - 1
        ...
        tags = torch.where(mask.unsqueeze(-1), best_tags_arr, oor_tag).permute(2, 1, 0)
        scores = (end_score/(seq_ends+1).view(batch_size,-1)).transpose(0,1)
        return tags, scores
